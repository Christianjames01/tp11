...
$serviceFee = $subtotal < 500 ? 50 : ($subtotal < 2000 ? 100 : 150);
...
$paymentReference = trim(isset($_POST['payment_reference'])? $_POST['payment_reference']: '');
    ...
    elseif (!$paymentMethod) $orderError = 'Please select a payment method.';
        ...
        // Recalculate with submitted qtys (they may have tweaked)
        $finalItems = [];
...
$finalSubtotal = array_sum(array_map(function($i) { return $i['price_per_kg'] * $i['qty']; }, $finalItems));
        ...
        $finalService     = $finalSubtotal < 500 ? 50 : ($finalSubtotal < 2000 ? 100 : 150);
        $finalPlatformFee = round($finalSubtotal * 0.05, 2);
        $finalFarmerPayout= round($finalSubtotal - $finalPlatformFee, 2);
        $finalGrand       = $finalSubtotal + $finalDelivery + $finalService;
        ...
        try {
            // 1 order record for all items from this farmer
            $oStmt = $pdo->prepare("INSERT INTO orders (buyer_id, farmer_id, status, total_amount, platform_fee, farmer_payout, delivery_fee, service_fee, distance_km, delivery_address, notes, payment_method, payment_reference, proof_of_payment) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $oStmt->execute([
...
setFlash('success', "Order #$orderId placed! {$farmName} will confirm soon. 🌾");
...
<div>
    <div style="font-weight:800;color:white;font-size:1rem;display:flex;align-items:center;gap:6px;">
        ...
        <?php if ($coFPrem): ?>
        <span style="background:linear-gradient(135deg,#78350f,#d97706);color:white;font-size:.55rem;font-weight:800;padding:2px 8px;border-radius:99px;letter-spacing:.04em;">⭐ PREMIUM</span>
        <?php endif; ?>
    </div>
    <div style="font-size:.75rem;color:rgba(255,255,255,.75);">📍 <?= sanitize($farmer['location']) ?> · <?= count($farmerItems) ?> product<?= count($farmerItems) > 1 ? 's' : '' ?> · 1 delivery</div>
                        ...
                        <div style="margin-left:auto;">
                            <span style="background:rgba(255,255,255,.15);color:white;border:1px solid rgba(255,255,255,.3);border-radius:99px;padding:4px 12px;font-size:.7rem;font-weight:800;">
                                <i class="fa-solid fa-truck me-1"></i>Combined Delivery
                            </span>
                        </div>
                    </div>
                        ...
                        <div style="display:flex;align-items:center;gap:12px;padding:1rem 1.25rem;border-bottom:1px solid var(--border);">
                            ...
                            <div style="display:flex;align-items:center;gap:5px;">
                                ...
                                <input type="number"
                                       ...
                                       data-min="<?= $item['min_order_kg'] ?>"
                                       data-max="<?= $item['stock_kg'] ?>"
                                       oninput="recalc()"
                ...
                <div class="gl-card mb-3">
                    <div class="gl-card-body">
                        <h6 style="font-weight:800;margin-bottom:1rem;"><i class="fa-solid fa-location-dot text-green me-2"></i>Delivery Address</h6>
                    ...
                    <div class="gl-card-body">
                        ...
                        <div id="pm_detail_wrap" style="display:none;margin-bottom:10px;">
                            ...
                            <div id="pmd_bank" class="pmd-panel" style="display:none;background:#EEF0FF;border:1.5px solid #C5CCFF;border-radius:14px;padding:1rem;">
                                ...
                                <div style="font-size:.78rem;margin-bottom:3px;">BPI: <strong style="font-family:monospace;">9876-5432-10</strong> — GreenLink Inc.</div>
                            ...
                            <div id="pmd_cod" class="pmd-panel" style="display:none;background:#FFFBEE;border:1.5px solid #FFE599;border-radius:14px;padding:1rem;">
                                ...
                                <div style="font-size:.72rem;color:#92400E;">💡 Farmer will contact you to arrange delivery. Payment on receipt.</div>
                                <input type="hidden" name="payment_reference" value="COD">
                            </div>
                        </div>

                        <!-- Proof upload (shown for non-COD) -->
                ...
                <div class="gl-card" style="position:sticky;top:80px;">
                    <div class="gl-card-body">
                        ...
                        <div style="display:flex;flex-direction:column;gap:7px;margin-bottom:.75rem;">
                            ...
                            <div style="display:flex;justify-content:space-between;font-size:.85rem;">
                                <span style="color:#3b82f6;">
                                    ...
                                    Delivery (<?= number_format($distanceKm,1) ?>km)
                                    <?php if (count($farmerItems) > 1): ?>
                                    <span style="font-size:.62rem;background:#dcfce7;color:#15803d;border-radius:99px;padding:1px 6px;font-weight:800;margin-left:3px;">combined</span>
                                    <?php endif; ?>
                                </span>
                                <strong style="color:#3b82f6;" id="sumDelivery">
                                    <?= $deliveryFee > 0 ? '₱'.number_format($deliveryFee,2) : 'FREE' ?>
                                </strong>
                            </div>
                            ...
                            <?php else: ?>
                            <div style="display:none;flex:justify-content:space-between;font-size:.75rem;background:#dcfce7;border-radius:8px;padding:4px 10px;" id="bulkRow">
                            ...
                            <div style="font-size:.68rem;color:#94a3b8;background:#f8f9fa;border-radius:8px;padding:6px 10px;line-height:1.7;">
                                <div><i class="fa-solid fa-circle-info" style="color:#7c3aed;"></i> Service: ₱50 (&lt;₱500) · ₱100 (₱500–₱1,999) · ₱150 (₱2,000+)</div>
                                <?php if ($distanceKm !== null): ?>
                                <div><i class="fa-solid fa-circle-info" style="color:#3b82f6;"></i> Delivery based on <?= number_format($distanceKm,1) ?>km · combined weight</div>
                                <?php endif; ?>
                                <div><i class="fa-solid fa-circle-info" style="color:var(--primary);"></i> 5% farmer commission deducted from farmer payout</div>
                            </div>
                        </div>
                        ...
                        <div style="background:var(--pale-green);border-radius:12px;padding:.9rem 1rem;margin-bottom:1.1rem;">
                            ...
                            <?php if (count($farmerItems) > 1): ?>
                            <div style="font-size:.68rem;color:#16a34a;font-weight:700;margin-top:4px;">
                                ✅ <?= count($farmerItems) ?> products combined into 1 delivery
                            </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <a href="cart_view.php" class="btn-outline-green w-100 justify-content-center mt-2" style="font-size:.82rem;padding:.6rem;">
                            ← Back to Cart
                        </a>
                    </div>
                </div>
...
function recalc() {
    ...
    // Update per-item subtotals and summary
